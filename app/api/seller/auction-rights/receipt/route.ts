...
<body>
  ...
  <table>
    ...
    <tbody>
      <tr>
        <td>Müzayede Hakkı (${p.quantity} adet × ${Number(p.unitPrice).toLocaleString('tr-TR')} ₺)</td>
        <td style="text-align:right;font-family:monospace;">${Number(p.totalAmount).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺</td>
      </tr>
      <tr>
        <td>Onay Tarihi</td>
