...
export function SellerDashboard() {
  ...
  const [auctionForm, setAuctionForm] = useState({
    ...
    liveOnly: false, buyerPremiumRate: null as number | null, // null = profil varsayılanı (yalnızca V2/Kontör'de kullanılır)
  });
  ...
  // V2 (Kontör): "Bu Müzayede İçin Alıcı Komisyonu" alanına, profil varsayılanını GERÇEK bir sayı
  // olarak bir kez yazıyoruz (V3/Hizmet Bedeli'nde bu alan hiç gösterilmez, admin belirler).
  useEffect(() => {
    ...
    try {
      ...
      const res = await fetch('/api/auctions', {
        ...
        body: JSON.stringify({
          ...
          buyerPremiumRate: auctionForm.buyerPremiumRate ?? undefined, // V3'te API zaten yok sayar
          status: 'DRAFT',
        }),
      });
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowCreateAuction(false)}>
            ...
            >
              ...
              <form onSubmit={handleCreateAuction} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    {paymentMode === 'DIRECT' && revenueModel === 'HIZMET_BEDELI' ? (
                      <>
