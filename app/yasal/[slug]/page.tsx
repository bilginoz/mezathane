...
const LEGAL_PAGES: Record<string, { title: string; icon: string; sections: { heading: string; text: string }[] }> = {
  ...
  'on-bilgilendirme': {
    ...
    sections: [
      ...
      { heading: '1. Aracı Bilgileri', text: 'TİCARİ UNVAN: MANES YAPI İNŞAAT SANAYİ TİCARET LİMİTED ŞİRKETİ\nADRES: Kemeraltı Mah. 155 Sk. Yaşam 1322 No: 38 İç Kapı No: 8 Marmaris/Muğla\nVERGİ DAİRESİ: Marmaris\nVERGİ NO: 6121803775\nMERSİS NO: 0612180377500001\nİLETİŞİM: WhatsApp +90 530 042 29 39\nE-POSTA: bilgi@mezathane.tr' },
...
// ALICI ile SATICI arasındadır. Alıcı, satış bedelini + Hizmet Bedelini (+ %20 KDV) DOĞRUDAN
// satıcıya öder; platform ürün bedelini taraf olarak tahsil etmez. Hizmet Bedeli oranını
// (2026-08-08 kararı) SATICI DEĞİL, PLATFORM (admin) satıcı bazında belirler — tek bir Hizmet
// Bedeli kavramı vardır, "satıcı komisyonu" diye ayrı bir şey yoktur. Platform geliri KONTOR
// alt modelinde satıcının önceden aldığı "Müzayede Hakkı"dır (bkz. HIZMET_BEDELI_OVERRIDES altta).
...
const DIRECT_OVERRIDES: Record<string, Record<string, string>> = {
  ...
  'mesafeli-satis': {
    '6. Fiyat ve Ödeme': 'Müzayedede verilen pey (teklif) tutarı, eserin KDV dahil satış bedelidir. Müzayedeyi kazanan alıcı, satış bedeline ek olarak PLATFORM tarafından satıcı bazında belirlenen oranda Hizmet Bedeli ve bu bedel üzerinden %20 KDV öder. Ödenecek toplam tutar, teklif verme ekranında açıkça gösterilir. Bu ödemenin tamamı DOĞRUDAN SATICIYA yapılır; platform aracı hizmet sağlayıcı olup ürün bedelini taraf olarak tahsil etmez.\n\n• Ödeme, müzayede bitiminden itibaren belirtilen süre içinde tamamlanmalıdır.\n• Alıcı, toplam tutarı satıcının bildirdiği hesaba havale yoluyla peşin olarak öder.',
  ...
  'kullanim-kosullari': {
    '4. Teklif ve Ödemenin Bağlayıcılığı': '• Bir lota verilen pey (teklif) bağlayıcıdır; müzayedeyi kazanan alıcı, satış bedeli ile birlikte PLATFORM tarafından satıcı bazında belirlenen orandaki Hizmet Bedelini ve bu bedel üzerinden %20 KDV\'yi ödemekle yükümlüdür. Bu ödemenin tamamı doğrudan satıcıya yapılır; ödenecek toplam tutar teklif ekranında açıkça gösterilir.\n• Ödeme, müzayede bitiminden itibaren belirtilen süre içinde (varsayılan 5 gün; müzayede bazında 2-7 gün arasında değişebilir) satıcının hesabına yapılmalıdır.\n• Süresinde ödenmeyen siparişler iptal edilebilir, kullanıcı hesabı kısıtlanabilir ve cezai şart uygulanabilir.\n• Müzayede yoluyla yapılan satışlar yasa gereği cayma hakkı kapsamı dışındadır; ayrıntılar İptal ve İade Koşulları sayfasındadır.',
  },
};

// AŞAMA 3 (2026-08-07, sadeleştirme 2026-08-08): DIRECT içindeki HİZMET_BEDELİ gelir modeli için
// EK override katmanı. DIRECT_OVERRIDES'ın üstüne biner (sadece directRevenueModel==='HIZMET_BEDELI'
// iken uygulanır). Alıcının satıcıya ödemesi DEĞİŞMEZ (hâlâ doğrudan, tek bir "Hizmet Bedeli" —
// bkz. DIRECT_OVERRIDES'taki tanım) — burada eklenen SADECE şu: satıcı, alıcıdan zaten tahsil ettiği
// bu AYNI Hizmet Bedelini SONRADAN (haftalık) platforma öder; KONTOR'da bunun yerine önden
// "Müzayede Hakkı" alınır. İki ayrı bedel/oran YOKTUR — tek terim, iki yönlü aynı tutar.
