...
export function SocialShare({ title, url, description, text, className = '', compact, onClose }: SocialShareProps) {
  ...
  return (
    <div ref={ref} className={`${compact ? 'flex flex-col gap-1 rounded-xl border border-border bg-card p-2 shadow-xl min-w-[160px]' : 'flex items-center gap-1'} ${className}`}>
      {shareLinks.map((link) => (
        <a
          key={link.name}
          href={link.href}
          ...
          title={link.name}
          className={`rounded-lg p-2 text-muted-foreground transition-colors ${link.color} ${compact ? 'flex items-center gap-2 text-sm' : ''}`}
        >
          {link.icon}
      ...
      <button
        onClick={handleCopy}
        title="Link kopyala"
        className={`rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors ${compact ? 'flex items-center gap-2 text-sm' : ''}`}
      >
        {copied ? <Check className="h-4 w-4 text-green-500" /> : <Link2 className="h-4 w-4" />}
        {compact && <span>{copied ? 'Kopyalandı!' : 'Link Kopyala'}</span>}
      </button>
    </div>
  );
}
