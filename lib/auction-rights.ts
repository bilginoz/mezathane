...
export async function grantGiftRight(
  ...
  const qty = Math.max(1, Math.min(AUCTION_RIGHT_MAX_QTY, Math.floor(quantity)));
  const validity = Math.max(1, Math.min(365, Math.floor(days)));
  const now = new Date();
  const expiresAt = new Date(now.getTime() + validity * 24 * 60 * 60 * 1000);
  await prisma.auctionRightPurchase.create({
    data: {
      ...
      approvedAt: now,
      approvedById,
      adminNote: (adminNote?.trim()?.slice(0, 500)) || 'Admin tarafından hediye edilen müzayede hakkı',
    },
  });
}
