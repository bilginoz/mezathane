...
import { buildBuyerLedger, buildSellerLedger, buildPlatformLedger, type LedgerResult } from '@/lib/ledger';
...
export async function resolveLedger(scope: string | null, type: string | null, id: string | null): Promise<ResolveOutcome> {
  ...
  if (scope === 'admin') {
    ...
    if (type === 'platform') return { ok: true, data: await buildPlatformLedger() };
    if (!id) return { ok: false, status: 400, error: 'id gerekli' };
    if (type === 'buyer') {
      const r = await buildBuyerLedger(id);
      return r ? { ok: true, data: r } : { ok: false, status: 404, error: 'Bulunamadı' };
    }
    if (type === 'seller') {
